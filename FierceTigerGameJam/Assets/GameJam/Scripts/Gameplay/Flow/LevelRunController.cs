...
namespace GameJam.Gameplay.Flow
{
    ...
    public sealed class LevelRunController : MonoBehaviour
    {
        ...
        [Tooltip("Read at pick time: the equipped cannon level's ammoBonus is added to the "
                 + "map's bullet budget (garage shows it as AMMO +N).")]
        [SerializeField] private GameJam.Gameplay.Combat.VehicleLoadout vehicleLoadout;
        ...
        private void ResolveMapRules()
        {
            ...
            // The equipped cannon's level tops the budget up (Falcon 2026-09-03: base 15 on
            // every campaign map, plus AMMO +0..+8 across the garage ladder). The bonus rides
            // whatever base the map authored, so the tutorial's tiny budget grows only by the
            // starter gun's +0.
            if (vehicleLoadout != null && vehicleLoadout.Selected != null)
            {
                BulletPickLimit += vehicleLoadout.Selected.ResolveAmmoBonus(vehicleLoadout.SelectedLevel);
            }
        }
