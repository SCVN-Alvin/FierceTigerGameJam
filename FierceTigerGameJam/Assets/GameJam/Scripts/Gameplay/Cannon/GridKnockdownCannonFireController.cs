...
namespace GameJam.Gameplay.Cannon
{
    public sealed class GridKnockdownCannonFireController : MonoBehaviour
    {
        ...
        /// <summary>
        /// Arm the NEXT shot to fire this many rounds (2 = Double, 3 = Triple). freeAmmo is for
        /// the FREE intro popup (Falcon 2026-09-03: "vi la free nen phat do ko ton dan") - the
        /// whole burst costs nothing; a paid ads/shop charge later leaves it false and every
        /// round pays its own bullet.
        /// </summary>
        ...
        private void Fire(Vector3 muzzlePosition, Vector3 direction)
        {
            ...
            if (armedBoostRounds > 1)
            {
                ...
                freeCharge = armedBoostFree;                // the intro gift costs nothing at all
            ...
            // Sequence firing, done right (Falcon 2026-09-03): a single tap's ball LEAVES the
            // current barrel in the rotation, but it flies the CENTRE-LINE solve - the lateral
            // barrel gap is handed to the projectile as a muzzle blend and slid away over the
            // first ~0.12s of flight. Straight-looking flight, exact landing, and the barrels
            // still take turns. (Yawing the velocity at the barrel instead - the 02v approach -
            // made singles fly visibly diagonal and clip neighbours at close range.)
            Vector3[] spawns = ResolveBurstSpawns(muzzlePosition, direction, rounds);
            Vector3[] directions;
            Vector3 blendOffset = Vector3.zero;
            if (rounds == 1)
            {
                directions = new[] { direction };
                blendOffset = muzzlePosition - spawns[0];   // barrel -> centre line
            }
            else
            {
                directions = ResolveBurstDirections(spawns, muzzlePosition, direction);
            }

            if (!FireSingle(spawns[0], directions[0], share, !freeCharge, blendOffset))
            {
                return;
            }
        ...
        {
            // The offset basis is anchored to the WORLD, not to the shot: the cannon model does
            // not pitch with the aim, so a barrel that sits 0.14 above the centre must spawn its
            // round 0.14 straight up, whatever the launch angle. The old arc-relative basis
            // (up = Cross(right, direction)) leaned backwards on upward aims, and the stacked
            // gun's second round appeared hanging in the air beside the muzzle instead of
            // leaving the lower barrel (Falcon 2026-09-03).
            ...
            Vector3 right = Vector3.Cross(Vector3.up, flat.normalized).normalized;
        ...
        private bool FireSingle(
            Vector3 muzzlePosition, Vector3 direction, float damageShare, bool spendAmmo,
            Vector3 muzzleBlendOffset = default)
        {
            ...
            if (muzzleBlendOffset != Vector3.zero)
            {
                projectile.SetMuzzleBlend(muzzleBlendOffset, 0.12f);
            }
