...
{
    /// <summary>
    /// What it costs to raise each vehicle to a given level. One row per vehicle, holding the
    /// price of every level that can be bought for it.
    ///
    /// Level 1 is where a vehicle starts once it has been purchased, so it is never listed and
    /// never has a price: the cheapest row is the price of reaching level 2.
    /// </summary>
    [CreateAssetMenu(menuName = "GameJam/Upgrade Vehicle Config", fileName = "UpgradeVehicleConfig")]
    public sealed class UpgradeVehicleConfig : ScriptableObject
    {
        [Serializable]
        public struct LevelPrice
        {
            [Tooltip("The level this purchase takes the vehicle TO, so 2 and up.")]
        ...
        public sealed class Entry
        {
            ...
            [Tooltip("One row per buyable level. A vehicle with no rows cannot be upgraded at all.")]
        ...
        /// <summary>
        /// Price of taking a vehicle to a level. Returns false when the vehicle is unknown or the
        /// level is not for sale, which includes level 1 because that is the starting level.
        /// </summary>
        public bool TryGetUpgradePrice(string vehicleId, int targetLevel, out int goldPrice)
        {
            goldPrice = 0;
            if (!TryGetEntry(vehicleId, out Entry entry) || entry.levels == null)
            {
                return false;
            }
        ...
        /// <summary>
        /// Highest level a vehicle can reach. Returns false when the vehicle is unknown; a listed
        /// vehicle with no buyable levels reports 1, since it is already at its ceiling.
        /// </summary>
        public bool TryGetMaxLevel(string vehicleId, out int maxLevel)
        {
            maxLevel = 0;
            if (!TryGetEntry(vehicleId, out Entry entry))
            {
                return false;
            }
        ...
        private bool TryGetEntry(string vehicleId, out Entry entry)
        {
            entry = null;
            ...
            EnsureLookup();
            return lookup.TryGetValue(vehicleId, out entry) && entry != null;
        }
            ...
            for (int i = 0; i < entries.Length; i++)
            {
                ...
                if (entry.levels != null)
                {
                    ...
                    for (int l = 0; l < entry.levels.Length; l++)
                    {
                        ...
                        {
                            Debug.LogWarning($"{name} lists level {price.targetLevel} of \"{entry.vehicleId}\" more than once; the first row wins.", this);
                        }
                    }
                }
