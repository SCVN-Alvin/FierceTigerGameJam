...
- **2026-09-03a** RE-PORT onto the new `main` (branch `Clone_Alvin-2`). The dev's 2026-09-02
  drop (garage 3D preview rig, artist icons, briefs 24-27) landed on main after our branch
  split, so all Falcon work was re-applied onto a fresh clone of main rather than merged:
  - 199 files carried over wholesale (scripts, configs, maps, prefabs, docs, textures).
  - `Gameplay.unity`: three-way merged (base = old main), clean - dev's garage/scene edits
    and our components (LevelScenery, GroundVisual, DragHint 1500100010, ReloadBar
    1500100012, ShotBoostIntro 1500100014, tapLabelFont) coexist.
  - `cannon_a/b/c.asset`: fused by hand - dev's NEW artist icons + our muzzleOffsets and
    reloadSeconds per level.
  - `BulletShopView.cs` / `VehicleShopView.cs`: taken from main AS-IS; our edits there
    existed only to feed the stopgap ShopModelShowcase, which is now RETIRED exactly as
    promised in the old note 01l - the dev's ModelPreviewRig/ModelPreviewView render-texture
    preview replaces it. ShopModelShowcase.cs (+meta) was NOT ported; the fire controller's
    DefaultProjectilePrefab accessor stays (harmless, may serve others).
  - THIS FILE renamed 26_ -> **30_** because the dev took brief numbers 24-27 (and moved
    Camera_Orbit/Mission_Config to 28/29). The dev's `26_Rocket_Arc.md` is his authored
    answer to our entry 02w (far shots fly low) - when implementing it, reconcile with 02w's
    comparison instruction; his `27_Balance_Pass.md` will touch prices/rewards that entries
    02i/02m/02r/02s already changed - the balance AI must read those first.
  - Old branch `Falcon/Clone_Alvin` stays on GitHub as the pre-port archive.

- **2026-09-03b** Single shots fly STRAIGHT again (Falcon: 2-barrel cannons "ban kha xeo, roi
  lech trai/phai"). Geometry lesson written down: leave-the-side-barrel + fly-straight +
  land-on-the-tap is pick-two. 02p (barrel alternation) + 02v (yaw convergence) chose barrel
  + landing, which made every single tap fly visibly diagonal. Now `Fire()` gives a 1-round
  shot the solved centre-line spawn and direction untouched - straight flight, exact landing;
  `muzzleCycle` no longer advances on singles. The authored `muzzleOffsets` + the 02v
  convergence remain EXACTLY as-is for Double/Triple bursts, where the multi-barrel spread is
  the point. NOTE: the dev's brief 26_Rocket_Arc rewrites this launch path anyway - whoever
  implements it should keep this single-shot rule (centre spawn) and re-apply per-barrel
  spread only to boosted bursts.

- **2026-09-03c** Sequence firing RESTORED, third design (Falcon: "toi muon no ban tung vien
  theo thu tu ma??" after 03b centre-only felt dead on the 3-barrel gun). The pick-two
  triangle is broken by a MUZZLE BLEND: the single tap's ball spawns at the current barrel
  (muzzleCycle walks the authored offsets again, advancing on every paid tap), flies the
  CENTRE-LINE solved velocity, and `GridKnockdownCannonProjectile.SetMuzzleBlend` slides the
  barrel-to-centre gap away over the first 0.12s (MovePosition steps in FixedUpdate, cleared
  on hit/launch). Result: each barrel visibly takes its turn, flight reads straight, landing
  is exactly the tap - and no diagonal path to clip the gun or neighbour blocks (the 02v
  yaw-at-the-barrel approach is now used ONLY by Double/Triple bursts, unchanged). For brief
  26_Rocket_Arc: keep this pattern - spawn at barrel, fly the centre solve, blend the gap.

- **2026-09-03d** Barrel-offset basis pinned to the WORLD (Falcon: on the stacked-barrel gun
  "dan 2 ben ko tu duoi bay len" - side/lower rounds materialised beside the muzzle). Cause:
  `ResolveBurstSpawns` built its offset frame from the shot direction (up = Cross(right,
  direction)), so on upward aims the "up" leaned back toward the camera and vertical barrel
  offsets drifted off the model, which never pitches with the aim. Now: right =
  Cross(worldUp, flattened direction), up = world up - a barrel authored 0.14 above centre
  spawns its round 0.14 straight up at any launch angle, singles' muzzle blend and burst
  convergence unchanged on top of it.

- **2026-09-03e** AMMO ladder (Falcon): every campaign map's base bullet budget drops 20 ->
  **15** (MapProgressionConfig, 28 entries; tutorial's 3 untouched), and the equipped
  cannon's LEVEL adds bonus bullets on top - new `VehicleDefinition.Level.ammoBonus` +
  `ResolveAmmoBonus`, seeded A **+0/+1/+2**, B **+3/+4/+5**, C **+6/+7/+8** (run budgets
  15..23). Applied in `LevelRunController.ResolveMapRules` via a new serialized
  vehicleLoadout ref (wired in the scene); every consumer of BulletPickLimit (pick screen,
  pool warm, fail-screen suggestion) inherits it automatically. Garage caption now reads
  "<NAME> - DAMAGE x2.60 - AMMO +8" (VehicleShopView.ResolveCaption; note the pack font
  renders the separator dots/multiply sign as blanks - the dev's own DAMAGE label already
  does). Balance note for brief 27: ammo is now ALSO an upgrade axis - the balance AI must
  weigh budgets as 15+bonus, not the flat 20 the old audit used.

- **2026-09-03f** The FREE Double/Triple intro costs no ammo (Falcon: "vi la free nen phat
  do ko ton dan"). `ArmShotBoost` gained a `freeAmmo` flag: the intro popup arms with it, and
  that one tap fires its whole burst with spendAmmo=false on every round - counter untouched,
  refused-when-empty gate unchanged, reload still arms after the shot. The flag clears with
